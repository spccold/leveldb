				...
				for (FileMetaData fileMetaData : files) {
					if (previousEnd != null) {
						Preconditions.checkArgument(
								...
								"Overlapping files %s and %s in level %s", previousFileNumber, fileMetaData.getNumber(),
								level);
					}
		...
		for (int level = 0; level < NUM_LEVELS; level++) {
			for (FileMetaData fileMetaData : getFiles(level)) {
				if (getInternalKeyComparator().compare(fileMetaData.getLargest(), key) <= 0) {
					...
					if (level > 0) {
						// Files other than level 0 are sorted by meta.smallest,
						// so no further files in this level will contain data for "ikey".
						break;
					}
				} else {
